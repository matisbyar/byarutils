...
function addToPool(methodToRun, args, attempts) {
  ...
  if (existingRequestIndex !== -1) {
    ...
    if (pool[existingRequestIndex].attempts <= 0) {
      ...
      log("ERROR", "Error Handler", "Error Handler Service: Failed to execute request. Removed from pool.");
      return;
    }

    log("WARNING", "Error Handler", `Error Handler Service: Updated request in pool. ${pool[existingRequestIndex].attempts} attempts left.`);
  } else {
    // Add a new request to the pool
    pool.push({ methodKey, methodToRun, args, attempts });
    log("WARNING", "Error Handler", `Error Handler Service: Added request to pool. ${attempts} attempts left.`);
  }
}
...
async function processPool() {
  if (pool.length === 0) {
    log("INFO", "Error Handler", "Error Handler Service: No errors awaiting resolution.");
    return;
  }
